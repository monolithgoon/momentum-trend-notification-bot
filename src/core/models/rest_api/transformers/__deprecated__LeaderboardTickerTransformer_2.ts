import { ILeaderboardTickerSnapshot } from "../__deprecatd__ILeaderboardTickerSnapshot.interface";
	...
	implements NormalizedRestTickerSnapshotTransformer<ILeaderboardTickerSnapshot>
{
	transform(snapshot: NormalizedRestTickerSnapshot): ILeaderboardTickerSnapshot {
		...
		} satisfies Rankings; // ✅ TS verifies completeness & exact keys
		...
		const out = {
			...
			// Default to -1, will be updated during merge & ranking steps
			num_consecutive_appearances: -1,
			num_consecutive_absences: -1,
			rankings, // ✅ now a complete Rankings object
			aggregate_kinetics_rank: -1,
			leaderboard_rank: -1,
		} satisfies ILeaderboardTickerSnapshot; // ✅ end-to-end check
