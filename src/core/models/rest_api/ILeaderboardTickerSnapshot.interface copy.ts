...
//   // Kinetics (derivedProps, mutable during enrich)
...
// The input shape you pass into the calculator
export interface IKineticsPipelineInput extends IKineticsComputationFields {}

// Make the big snapshot include those same fields (keeps types in sync)
export interface ILeaderboardTickerSnapshot_2
  extends BaseInternalTickerSnapshot,
    IKineticsComputationFields {
  // Kinetics (derivedProps)
  pct_change_velocity__ld_tick: number;
  pct_change_acceleration__ld_tick: number;
  volume_velocity__ld_tick: number;
  volume_acceleration__ld_tick: number;
