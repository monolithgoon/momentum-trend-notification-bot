...
export function makeEngine(cfg: LeaderboardCfg, ports: { storage: StoragePort; log: Logger; metrics: Metrics }) {
	const kinetics = deriveKineticsCfg(cfg.windows, cfg.lookbackCap);
	...
	const stages: Stage[] = [
		...
		new FetchHistoryStage(ports.storage, kinetics.lookbackSamples),
		new ComputeSignalsStage(
			deriv,
			{
				velWindowSamples: kinetics.velWindowSamples,
				accWindowSamples: kinetics.accWindowSamples,
				minSamplesForAccel: kinetics.minSamplesForAccel,
