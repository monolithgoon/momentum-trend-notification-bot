...
import { KineticsCalculator } from "@services/leaderboard/KineticsCalculator";

export async function computeNewBatchKinetics(
	newBatchMap: Map<string, ILeaderboardTickerSnapshot>,
	leaderboardTag: string,
	storage: ILeaderboardStorage
): Promise<Map<string, ILeaderboardTickerSnapshot>> {
	/**
	...
	const enrichedBatchMap: Map<string, ILeaderboardTickerSnapshot> = new Map(); // Init. map to hold enriched ticker data
	...
	for (const snapshot of snapshots) {
		try {
			...
			const enrichedSnapshot: ILeaderboardTickerSnapshot = {
				...snapshot,
