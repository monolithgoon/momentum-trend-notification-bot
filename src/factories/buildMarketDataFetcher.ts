...
import { PolygonMarketDataFetcher_2 } from "@data_vendors/polygon/fetchers/polygonMarketDataFetcher_2";
import { PolygonTickerTransformer } from "@data_vendors/polygon/transformers/polygonTickerTransformer";
...
export function buildMarketDataFetcher(vendor: MarketDataVendors, strategyKey: string): MarketDataFetcher {
	switch (vendor) {
		...
		// return new EODHDMarketFetcher(new EODHDTransformer(), eodhdStrategyRegistry);
